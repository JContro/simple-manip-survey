...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Train conversation classifier')
        ...
        for conversation in tqdm(conversations, desc="Processing conversations"):
            ...
            while len(utterance_embeddings) < self.max_utterances:
                utterance_embeddings.append(
                    torch.zeros_like(utterance_embeddings[0]))
...
def save_fold_predictions(fold_metrics, binary_cols, save_dir, conversations, val_idx, df):
    ...
    temp_df = pd.DataFrame(
        {'conversation': [conversations[i] for i in val_idx]})
    ...
    if len(results_df) != len(temp_df):
        raise ValueError(
            f"Mismatch in merge: {len(results_df)} results vs {len(temp_df)} predictions")
...
def main():
    ...
    fold_info = get_folds(
        conversations, df, fold_pred_path='data/longformer_all_fold_predictions.csv')
    ...
    for (fold, train_idx, val_idx) in fold_info:
        ...
        for epoch in range(args.num_epochs):
            ...
            for i, col in enumerate(binary_cols):
                report = classification_report(
                    val_labels[:, i], val_preds[:, i], output_dict=True)
                if not args.disable_wandb:
                    wandb.log({
        ...
        save_path = save_fold_predictions(
            fold_metrics, binary_cols, save_dir, conversations, val_idx, df)
